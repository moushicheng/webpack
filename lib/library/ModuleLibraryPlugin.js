...
/**
 * Whether every export the module defines is a binding the library can export
 * natively, so the definitions written into the exports object are read by nobody.
 * A re-export or a renamed import is not: `renderStartup` reads those back off the
 * object, and `treatAsCommonJs` hands the object out as the default export.
 * @param {Module} module the entry module
 * @param {ModuleGraph} moduleGraph the module graph
 * @returns {boolean} true when the definitions may be taken over
 */
const definesOnlyNativeExports = (module, moduleGraph) => {
	const buildMeta = /** @type {JavascriptModuleBuildMeta} */ (module.buildMeta);
	if (buildMeta && buildMeta.treatAsCommonJs) return false;
	...
	if (moduleGraph.isAsync(module)) return false;
	const topLevelDeclarations =
		module.buildInfo && module.buildInfo.topLevelDeclarations;
	if (!topLevelDeclarations) return false;
	for (const dep of module.dependencies) {
		// A re-export or an `export default` has no declaration of its own to bind to,
		// so `renderStartup` reads its value back off the exports object.
		if (
			dep instanceof HarmonyExportImportedSpecifierDependency ||
			dep instanceof HarmonyExportExpressionDependency
		) {
			return false;
		}
		if (!(dep instanceof HarmonyExportSpecifierDependency)) continue;
		if (!topLevelDeclarations.has(dep.id)) return false;
	}
	return true;
};
...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			// The same for a plain entry, whose definitions come from a dependency template.
			// Placed after the module body, which its value bindings read.
			javascriptHooks.onDemandExportsGeneration.tap(
				PLUGIN_NAME,
				(module, runtime, source, placeAtEnd) => {
					const entries = libraryEntryModules.get(compilation);
					if (!entries || !entries.has(module)) return;
					if (!definesOnlyNativeExports(module, compilation.moduleGraph)) {
						return;
					}
					/** @type {BuildMeta} */
					const buildMeta = module.buildMeta || (module.buildMeta = {});
					...
					if (placeAtEnd) {
						const bindings =
							buildMeta.exportsBindingSourceByRuntime ||
							(buildMeta.exportsBindingSourceByRuntime = new Map());
						bindings.set(key, source);
					} else {
						const sources =
							buildMeta.exportsSourceByRuntime ||
							(buildMeta.exportsSourceByRuntime = new Map());
						// The `__esModule` marker and the definitions both land here, so
						// keep whichever arrived first rather than replacing it.
						const before = sources.get(key);
						sources.set(key, before ? `${before}${source}` : source);
					}
					return true;
				}
			);
			...
			compilation.hooks.additionalChunkRuntimeRequirements.tap(
				...
				(chunk, set, { chunkGraph, codeGenerationResults }) => {
					...
					const runtimeKey = getRuntimeKey(chunk.runtime);
					const buildMeta = module.buildMeta;
					const takenOver =
						(buildMeta &&
							buildMeta.exportsSourceByRuntime &&
							buildMeta.exportsSourceByRuntime.has(runtimeKey)) ||
						(buildMeta &&
							buildMeta.exportsBindingSourceByRuntime &&
							buildMeta.exportsBindingSourceByRuntime.has(runtimeKey));
					if (!takenOver) return;
	...
	renderStartup(source, module, renderContext, { options, compilation }) {
		...
		// Taken over by `onDemandExportsGeneration` above, and re-emitted by
		// `renderModuleContent` only where the module is wrapped — which is not here.
		const bindingsTakenOver = Boolean(
			module.buildMeta &&
			module.buildMeta.exportsBindingSourceByRuntime &&
			module.buildMeta.exportsBindingSourceByRuntime.has(
				getRuntimeKey(chunk.runtime)
			)
		);
		...
		outer: for (const exportInfo of exportInfos) {
			...
			if (definition) {
				...
				// The declaration is only there for the definitions written into it; with
				// those taken over (and not re-emitted here) nothing reads the object.
				if (!bindingsTakenOver) needExportsDeclaration = true;
			} else {
	...
	renderModuleContent(
		...
		const exportsSource =
			buildMeta &&
			buildMeta.exportsSourceByRuntime &&
			buildMeta.exportsSourceByRuntime.get(runtimeKey);
		const bindingSource =
			buildMeta &&
			buildMeta.exportsBindingSourceByRuntime &&
			buildMeta.exportsBindingSourceByRuntime.get(runtimeKey);
